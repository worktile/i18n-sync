...
const config       = {
    ...
    options: [
        {
            simple: '-r',
            complete: '--rootPath [value]',
            desc: '同步文件目录,默认为"www/i18n/locales",可指定env=server则为server/i18n/locales'
        },
        {
            simple: '-s',
            complete: '--source [value]',
            desc: '同步原文件名,默认为"zh-cn"'
        },
        {
            simple: '-t',
            complete: '--targets <items>',
            desc: '同步目标文件名,文件名以","号隔开,默认为"en-us","zh-tw"',
            type: (val)=> val.split(',')
        },
        {
            simple: '-S',
            complete: '--stringifySpace <n>',
            desc: '空格数,默认为2',
            type: parseInt
        }
    ]
...
_.forEach(config.options, function (item) {
    program.option(`${item.simple}, ${item.complete}`, item.desc, item.type)
});
...
program.arguments('<env>')
    ...
    .usage(config.usage)
    .action(function (env) {
        let rootPath   = program.rootPath || 'www/static/i18n';
        let fileName   = program.source || 'zh-cn';
        stringifySpace = program.stringifySpace || stringifySpace;

        if (env === 'server') {
            otherLanguages = program.targets || ['en-us', 'zh-tw'];
            rootPath       = program.rootPath || 'server/i18n/locales';

            if (path.extname(fileName) !== '.json') {
                fileName += '.json';
            }
            const filePath = path.join(rootPath, fileName);
            fileOrDirectoryExists(filePath);
            _.forEach(otherLanguages, function (item) {
                fileOrDirectoryExists(path.join(rootPath, item + '.json'))
            });
            confirmCompareI18nFile(filePath)
        } else {
            fileOrDirectoryExists(path.join(rootPath, fileName));
            _.forEach(otherLanguages, function (item) {
                fileOrDirectoryExists(path.join(rootPath, item))
            });
            const rootFileOrFolders = glob.sync(path.join(rootPath, fileName, '*'));
            inquirer.prompt([
                {
                    type: 'list',
                    name: 'name',
                    message: 'What do you sync i18n?',
                    choices: rootFileOrFolders
                }
            ]).then(function (answers) {
                if (answers.name.indexOf('.json') >= 0) {
                    confirmCompareI18nFile(answers.name);
                } else {
                    const files = glob.sync(`${answers.name}/*.json`);
                    inquirer.prompt([
                        {
                            type: 'list',
                            name: 'name',
                            message: 'What do you sync i18n?',
                            choices: _.map(files, function (file, index) {
                                return index + ') ' + file;
                            })
                        }
                    ]).then(function (answers) {
                        confirmCompareI18nFile(answers.name.replace(/\d+\) /, ''));
                    })
                }
            });
        }
...
function confirmCompareI18nFile(file) {
    inquirer.prompt([
        {
            type: 'confirm',
            name: 'confirm',
            message: `Are you sync i18n - ${file}?`,
            default: function () {
                return true;
            }
        }
        ...
        if (result.confirm) {
            compareI18nFile(file);
        }
    });
}

function compareI18nFile(zhFilePath) {
    const zhContent = fs.readFileSync(zhFilePath);
    _.forEach(otherLanguages, function (otherLanguage) {
        const originalZHJson     = JSON.parse(zhContent);
        var languagePath         = zhFilePath.replace('zh-cn', otherLanguage);
        const languageContent    = fs.readFileSync(languagePath);
        const languageJsonObject = JSON.parse(languageContent);
        for (let key in originalZHJson) {
            recursionTranslate(otherLanguage, originalZHJson, languageJsonObject, key);
        }
        fs.writeFileSync(languagePath, JSON.stringify(originalZHJson, null, stringifySpace));
        console.log(`sync ${languagePath} file successfully.`);
    });
}

function recursionTranslate(language, source, target, key) {
    if (_.isString(source[key])) {
        if (target[key]) {
            source[key] = target[key];
        } else {
            source[key] = translate(language, source[key]);
        }
    } else {
        for (let secondKey in source[key]) {
            recursionTranslate(language, source[key], target[key] || {}, secondKey)
        }
    }
}

function translate(language, text) {
    if (language === 'zh-tw' || language === 'ja-jp') {
...
function fileOrDirectoryExists(fileOrDirectory) {
    if (!fs.existsSync(fileOrDirectory)) {
        console.log(`no such file or directory, please make sure the ${fileOrDirectory} exists`);
        process.exit(1);
    }
}